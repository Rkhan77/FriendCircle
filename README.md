...
Open http://127.0.0.1:3000. The default demo runs only on loopback and uses **fictional people and locations**. Changes persist in `.data/demo.json`. Demo friends do not simulate replies or successful AI checks. Uploading a photo for Circle ID `ella` creates a manual-review request; platform admins review it from the separate `/admin` dashboard. Reviewer approval does not auto-accept a friendship. Mia and Ava are mutual friends, so you can create a demo local group with them.
...
- Member account access at `/auth` accepts an email address or an international-format phone number. Existing accounts can use a password or request a one-time code; new members can sign up and then complete email or phone confirmation. Supabase sessions use secure HTTP-only cookies in production and JWKS verification. Adult onboarding, explicit photo consent, and public or private profile visibility still apply. Only public profiles in the viewer's current suburb are discoverable when they share at least one profile interest; private profiles require accepted friendship. Newly visible local public profiles, newly available local restaurant offers, and friends entering the same suburb or coming within 200 m trigger in-app notifications. Connected friends sharing their suburb contribute to persistent achievements at 3, 5, and 8 distinct suburbs; extra perks are planned but not active.
...
- Dedicated platform admin sign-in at `/admin/login`, leading to `/admin`. Admin and restaurant redemption controls are absent from the member interface. In live mode, the Supabase password session is issued only after the account UUID matches `ADMIN_USER_IDS`. The dashboard has a restaurant list showing discount rate, invitations, acceptances, and redemptions against each venue's limit; Manage opens a pop-out editor for rates, offer limit, and partner terms. Register restaurant opens `/admin/restaurants/new` to create an unpublished draft, with a **Register & send invite** action; the Manage pop-out can resend an invitation. Live invitations are emailed through Supabase, while the local demo displays a copyable sample link. Only accounts assigned to a restaurant can use `/partner`; in the local demo, an owner must open the admin-issued invitation link, because the public sample-partner shortcut is removed. Partners can edit contact details, ask an admin to approve restaurant name or location changes, create multiple scheduled offers, and redeem diner codes from their own dashboard. The Manage pop-out can also remove a venue, hiding it from active views and expiring unused offers while preserving aggregate reporting history. The dashboard resolves community reports and shows aggregate current activities, verified meet counts/duration, daily meet trend, and offer acceptance/redemption totals. The local demo uses a labeled preview entry with fictional accounts; it has no real admin credentials.
- Inviting friends to a meal opens a searchable selection window. Friends with recent direct messages, verified meetings, or accepted meal invitations appear first. Recommendation scoring uses interaction metadata, not chat contents or precise coordinates.
...
2. Create a Supabase project with an asymmetric JWT signing key (JWKS), enable email/password and phone authentication as needed, and set `SUPABASE_URL` and `SUPABASE_ANON_KEY`. Add `${PUBLIC_ORIGIN}/auth/complete` and `${PUBLIC_ORIGIN}/partner/accept` to the allowed redirect URLs. Configure an SMS provider before testing real phone codes, and an email OTP template if you want typed email codes in addition to sign-in links. Set the server-only `SUPABASE_SECRET_KEY` (or `SUPABASE_SERVICE_ROLE_KEY`) to email partner invitations; never expose this key to client code. Users can register, confirm their email or phone, attest 18+, and create a profile. Password recovery is not yet implemented. See the [Supabase sign-up](https://supabase.com/docs/reference/javascript/auth-signup), [OTP](https://supabase.com/docs/reference/javascript/auth-signinwithotp), [admin invite](https://supabase.com/docs/guides/auth/users), and [phone login](https://supabase.com/docs/guides/auth/phone-login) documentation.
...
10. For meal offers, obtain written approval from each restaurant for the exact **2/3/4+ diner discount tiers**, total redemption limit, who funds them, exclusions, dates, and redemption process. In the admin restaurant editor, complete the draft with venue coordinates and an invited restaurant owner account, then confirm approval to publish. A code can be redeemed only by the assigned owner account. The owner should verify the accepted diners are present before redeeming. Validate GPS integrity and restaurant procedures before enabling valuable real-world discounts.
