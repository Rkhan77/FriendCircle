...
import { ArrowLeft, CalendarDays, CheckCheck, Clock3, LogOut, MapPin } from "lucide-react";
...
export default function PartnerDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (/sign in|no restaurant|assigned/i.test(message)) window.location.replace(appHref("/partner/login"));
  ...
  return <main className="platform-dashboard partner-dashboard">
    <header className="admin-topbar"><div className="admin-topbrand"><BrandLogo /><b>partner</b></div><div><a href={appHref("/app")}><ArrowLeft size={16} /> App</a><button onClick={async () => { await api("/logout", { method: "POST" }); window.location.assign(appHref("/partner/login")); }}><LogOut size={16} /> Sign out</button></div></header>
