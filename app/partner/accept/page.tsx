...
export default function AcceptPartnerInvite() {
  ...
  useEffect(() => {
    ...
    window.history.replaceState({}, "", "/partner/accept");
    ...
    api("/partner/accept", { method: "POST", body: JSON.stringify(demoInvite ? { token: demoInvite } : { accessToken }) })
      .then(() => window.location.replace("/partner"))
      .catch((cause) => setError((cause as Error).message));
  }, []);
  return <main className="admin-auth"><div className="admin-auth-card"><h1>Opening your partner space…</h1>{error && <div className="admin-error" role="alert">{error}</div>}<a href="/partner/login">Partner sign in</a></div></main>;
}
